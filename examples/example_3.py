...
def salaries_new_emp_1(new_employees):
    """
        Returns a data frame containing for each new employee his salary when he entered
        the population.
    ...
    for i in range(n):
        strate = new_employees.loc[i, 'group']
        if strate > 11:
            strate = strate - 11
        if strate in entry_salaries:
...
def f_pension_th(data, salaries , types_):
    ...
    # pension théorique
    pth = (sal_moyen*(2018-data['Year_employment']+year_retirement-year_entrance))*0.025
...
def f_pension_min(data, salaries , types_):
    ...
    # date engagement
    # date_eng = datetime.strptime(data['Year_employment'], '%d/%m/%Y')
    # if date_eng.year >= 2002:
    #     return pth

    if data['Year_employment'] >= 2002:
        return pth

    # pension catégorielle par strate
    pension_cat = {0:0,1:18000, 2:18000, 3:24000, 4:24000, 5:30000, 
    ...
    if data['Year_employment'] < 1995:
        return min(pension_cat[strate], pth/2)
    else:
        temp = max(pension_cat[strate], pth/2)
        if pth < temp:
            return pth/2
        else:
            return max(pension_cat[strate], pth/2)
...
# capital is earned once, so we set it's evolution to -1
...
eff.individual_employees_numbers(numbers_[0])[0].to_csv('./results/projected_employees.csv', sep=';', index=False, decimal=',')
